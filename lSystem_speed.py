...
def main():
    ...
    if len(sys.argv) > 1:
        configFilename, iterations, exportImageName, showDrawProcess, noProgressbar, closeAfterDrawing, useRandomConfig = processCommandlineArguments()
...
def turtleRunMainLoop(screen, turt, lSystem, translations, noProgressbar):
    ...
    for i in range(5):
        ...
        timeing.append(stop_time-start_time)
    
    print(sum(timeing)/len(timeing))
...
def getTurtlePosition(screen, turt, turtPos):
    currentPos = turt.pos()
    #dit is kort omdat het sneller is
    minX, minY, maxX, maxY = min(currentPos[0], turtPos[0]), min(currentPos[1], turtPos[1]), max(currentPos[0], turtPos[2]), max(currentPos[1], turtPos[3])
